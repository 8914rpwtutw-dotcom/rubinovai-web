    ...
    async with AsyncSessionLocal() as session:
        ...
        if user.is_banned:
            await message.answer(
                f"⛔️ Аккаунт заблокирован.\nПричина: {user.ban_reason or 'Не указана'}\n\n"
                f"Для апелляции используйте: `/ticket Ваш текст`", 
                parse_mode="Markdown"
            )
            return
        ...
        await message.answer(
            ...
            f"Статус: **{vip_str}**\n"
            f"⏱ Введите его на сайте для входа.", 
            parse_mode="Markdown",
...
async def create_ticket(message: types.Message, command: CommandObject):
    if not command.args:
        await message.answer("Формат: `/ticket Текст проблемы`", parse_mode="Markdown")
        return
...
async def reply_ticket(message: types.Message, command: CommandObject):
    ...
    try:
        ...
        await message.answer("Ошибка. Формат: `/reply <ID> <текст>`")
...
async def admin_panel(message: types.Message):
    ...
    async with AsyncSessionLocal() as session:
        ...
        text = (
            "👑 **Админ-панель**\n\n"
            f"📊 Всего: `{total}` | VIP: `{vip}` | Забанено: `{banned}`\n\n"
            "**Команды:**\n"
            "• `/vip <ID>` | `/unvip <ID>`\n"
...
async def list_users(message: types.Message):
    ...
    async with AsyncSessionLocal() as session:
        ...
        text = "👥 **Пользователи:**\n\n"
        for u in users:
            text += f"• ID: `{u.telegram_id}` | @{u.username or 'no'} | VIP: {u.is_vip} | BAN: {u.is_banned}\n"
        await message.answer(text, parse_mode="Markdown")
...
async def give_vip(message: types.Message, command: CommandObject):
    ...
    async with AsyncSessionLocal() as session:
        ...
        user.is_vip = True
        await session.commit()
        await message.answer(f"⭐ VIP выдан пользователю `{user.telegram_id}`")
