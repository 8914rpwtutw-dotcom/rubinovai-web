    ...
    if "message" in data:
        ...
        if text.startswith("/start"):
            ...
            try:
                ...
                send_telegram_message(target_id, "ℹ️ Ваш VIP статус был снят администратором.")
            except Exception:
            ...
            try:
                ...
                send_telegram_message(target_id, "🚫 Вы были заблокированы администратором.")
            except Exception:
            ...
            try:
                ...
                send_telegram_message(target_id, "✨ Ваш аккаунт разблокирован администратором!")
            except Exception:
                send_telegram_message(chat_id, "Ошибка. Использование: `/unban ID`")
        else:
...
async def chat_endpoint(
    ...
    if not tg_user_id:
        raise HTTPException(status_code=401, detail="Требуется авторизация через Telegram в боковой панели")
...
<head>
    ...
    <style>
        ...
        * { box-sizing: border-box; margin: 0; padding: 0; font-family: 'Plus Jakarta Sans', -apple-system, sans-serif; -webkit-tap-highlight-color: transparent; }
        ...
        .brand h2 { font-size: 15px; font-weight: 700; color: #ffffff; letter-spacing: -0.3px; display: flex; align-items: center; gap: 6px; }
        ...
        #chat-container { 
            flex: 1; overflow-y: auto; padding: 24px 24px 140px 24px; display: flex; flex-direction: column; gap: 22px; max-width: 900px; width: 100%; margin: 0 auto; position: relative; z-index: 2;
        }
        ...
        .welcome-avatar-glow {
            position: relative; padding: 20px; border-radius: 28px; background: rgba(168, 85, 247, 0.04); border: 1px solid rgba(168, 85, 247, 0.15); box-shadow: 0 0 50px rgba(168, 85, 247, 0.12); margin-bottom: 4px;
        }
        ...
        .loader-box {
            ...
            border-radius: 18px 18px 18px 4px; padding: 14px 20px; font-size: 13.5px; color: var(--text-muted); backdrop-filter: blur(20px); box-shadow: 0 10px 30px rgba(0, 0, 0, 0.25);
        }
        ...
        #input-container { 
            max-width: 900px; margin: 0 auto; background: rgba(13, 16, 24, 0.8); backdrop-filter: blur(24px); -webkit-backdrop-filter: blur(24px);
        ...
        .mini-btn { 
            background: rgba(255, 255, 255, 0.02); border: 1px solid var(--border-color); color: var(--text-muted); padding: 10px; 
            border-radius: 12px; cursor: pointer; display: flex; align-items: center; justify-content: center; transition: all 0.2s ease; 
        }
        ...
        .btn-action { 
            background: var(--accent-gradient); color: #ffffff; border: none; border-radius: 12px; padding: 11px 20px; font-size: 12.5px; font-weight: 600; cursor: pointer; 
            transition: all 0.25s cubic-bezier(0.16, 1, 0.3, 1); flex-shrink: 0; display: flex; align-items: center; justify-content: center; min-width: 100px; box-shadow: 0 4px 20px rgba(99, 102, 241, 0.3);
        }
        .btn-action.cancel-mode { background: var(--cancel-bg); border: 1px solid var(--cancel-border); color: var(--cancel-color); }

        /* Оверлей бана */
        #ban-overlay {
            display: none; position: fixed; top: 0; left: 0; width: 100vw; height: 100dvh; background: rgba(4, 5, 8, 0.95);
            backdrop-filter: blur(20px); z-index: 100; flex-direction: column; align-items: center; justify-content: center; text-align: center; padding: 24px;
        }
...
<body>
    ...
    <div id="ban-overlay">
        ...
        <p>Ваш аккаунт был заблокирован администратором в Telegram боте. Использование сервиса недоступно.</p>
    </div>
    ...
    <div id="sidebar">
        <div class="brand">
            ...
            <div>
                ...
                <span id="user-display-status">Авторизуйтесь через ТГ</span>
            </div>
        </div>
        ...
        <div class="sidebar-footer">
            <div class="status-info">
                ...
                <span id="plan-text">Free план</span>
            </div>
        ...
        <div id="input-wrapper">
            <div id="input-container">
                ...
                <div class="input-row">
                    ...
                    <button class="mini-btn" onclick="triggerFileButton()" title="Прикрепить файл (VIP)">
    ...
    <script>
        let chats = JSON.parse(localStorage.getItem('rubinov_chats_main_v2') || '[]');
        let currentChatId = localStorage.getItem('rubinov_active_chat_main_v2') || null;
            ...
            try {
                ...
                if (data.logged_in) {
                    ...
                    if (isVip) {
                        ...
                        document.getElementById('plan-text').textContent = 'Free План';
                    }
                } else {
        ...
        function renderTelegramLoginButton() {
            ...
            script.setAttribute('data-telegram-login', 'rubinov_ai_bot'); // Укажите юзернейм вашего бота без @
        ...
        setInterval(checkUserStatus, 10000); // Проверка статуса/бана каждые 10 секунд
        checkUserStatus();
        ...
        function saveState() {
            localStorage.setItem('rubinov_chats_main_v2', JSON.stringify(chats));
            localStorage.setItem('rubinov_active_chat_main_v2', currentChatId);
            renderChats();
        }
        ...
        function triggerFileButton() {
            if (!isLogged) { alert("Сначала авторизуйтесь через Telegram в боковой панели!"); return; }
            if (!isVip) { alert("⭐ Отправка файлов доступна только для VIP пользователей!"); return; }
        ...
        async function sendMessage() {
            if (!isLogged) { alert("Сначала войдите через Telegram в боковой панели слева!"); return; }
